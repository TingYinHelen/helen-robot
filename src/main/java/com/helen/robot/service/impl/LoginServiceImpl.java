package com.helen.robot.service.impl;
...
import com.helen.robot.utils.enums.ResultEnum;
...
import com.helen.robot.utils.enums.parameters.LoginParaEnum;
import com.helen.robot.utils.enums.parameters.StatusNotifyParaEnum;
import com.helen.robot.utils.enums.parameters.UUIDParaEnum;
...
public class LoginServiceImpl implements ILoginService {
	...
	public LoginServiceImpl(CoreInfo coreInfo) {
		this.coreInfo = coreInfo;
		httpClient = coreInfo.getMyHttpClient();
		myHttpClient = coreInfo.getMyHttpClient();
	}
	...
	public boolean waitForLogin() {
		...
		params.add(new BasicNameValuePair(LoginParaEnum.UUID.para(), coreInfo.getUuid()));
		...
		while (!isLogin) {
			...
			try {
				...
				if (ResultEnum.SUCCESS.getCode().equals(status)) {
					...
					isLogin = true;
					this.coreInfo.setAlive(isLogin);
					break;
				}
	...
	public String getUuid() {
		...
		try {
			...
			if (matcher.find()) {
				if ((ResultEnum.SUCCESS.getCode().equals(matcher.group(1)))) {
					this.coreInfo.setUuid(matcher.group(2));
				}
			}
		} catch (Exception e) {
		...
		return this.coreInfo.getUuid();
	}

	@Override
	public boolean getQR(String qrPath) {
		qrPath = qrPath + File.separator + "QR.jpg";
		String qrUrl = URLEnum.QRCODE_URL.getUrl() + this.coreInfo.getUuid();
							...
							if (selector.equals("2")) {
								if (msgObj != null) {
									try {
										...
										msgList = MsgCenter.produceMsg(msgList, coreInfo);
