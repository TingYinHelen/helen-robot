...
import com.helen.robot.beans.RecommendInfo;
...
import com.helen.robot.utils.enums.VerifyFriendEnum;
...
public class MessageTools {
	...
	private final CoreInfo core;

	public MessageTools(CoreInfo core) {
		this.core = core;
	...
	public boolean sendMsgByNickName(String text, String nickName) {
		if (nickName != null) {
			String toUserName = this.core.getWechatTools().getUserNameByNickName(nickName);
			if (toUserName != null) {
				webWxSendMsg(1, text, toUserName);
				return true;
			}
	...
	public void webWxSendInvite(String nickName) {
		CoreInfo core = this.core;
		String toUserName = this.core.getWechatTools().getUserNameByNickName(nickName);
		String url = String.format(URLEnum.WEB_WX_SEND_MSG.getUrl(), core.getLoginInfo().get("url"));
		Map<String, Object> msgMap = new HashMap<>();
		msgMap.put("Type", 49);
		msgMap.put("Content",
				"<?xml version=\"1.0\"><msg><appmsg appid=\"wx91868160858ba7ba\"  sdkver=\"0\"><title>asa</title><des>sasa111</des><type>5</type><content>dasdasd</content><url>https://www.baidu.com</url></appmsg><appinfo><version>1</version><appname>2</appname></appinfo></msg>");
		msgMap.put("AppMsgType", 5);
		msgMap.put("FromUserName", core.getUserName());
		msgMap.put("ToUserName", toUserName == null ? core.getUserName() : toUserName);
		msgMap.put("Url", "https://www.baidu.com");
		msgMap.put("FileName", "标题");
		Map<String, Object> paramMap = core.getParamMap();
		paramMap.put("Msg", msgMap);
	...
	public boolean sendFileMsgByUserId(String userId, String filePath) {
		...
		if (responseObj != null) {
			...
			LOG.error("sendFileMsgByUserId 错误: ", data);
		}
		return webWxSendAppMsg(userId, data);
	}

	public boolean sendInviteByUserId(String nickName) {
		String toUserName = this.core.getWechatTools().getUserNameByNickName(nickName);
		Map<String, String> data = new HashMap<>();
		data.put("appid", Config.API_WXAPPID);
		data.put("title", "panda14");
